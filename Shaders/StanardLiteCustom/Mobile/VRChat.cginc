...
// _BICUBIC feature stripped: always use normal sampling.
...
// Variant that preserves baked GI (lightmaps/SH/lightprobes) but removes realtime direct light contribution.
// This is the safest way to get "baked + probes only" without touching baking/Meta passes.
inline half4 LightingStandardVRCNoRealtime(SurfaceOutputStandardVRC s, float3 viewDir, UnityGI gi)
{
    // Ensure direct light is treated as absent in the BRDF.
    gi.light.color = 0;
    gi.light.dir = 0;
    // Prevent SH/lightprobe "specular hack" / unity_IndirectSpecColor from washing everything out.
    gi.indirect.specular = 0;

    return LightingStandardVRC(s, viewDir, gi);
}

inline void LightingStandardVRCNoRealtime_GI(SurfaceOutputStandardVRC s, UnityGIInput data, inout UnityGI gi)
{
    // Variant of LightingStandardVRC_GI with the MinimumBrightness clamp removed.
    // This avoids "everything white" if MinimumBrightness is unset/garbage in certain generated code paths.
    // We also skip reflection probe / indirect specular work entirely.
    UnityGIInput dataNoRealtime = data;
    // Unity may pack per-vertex additional lights into data.ambient; disable that for "no realtime lights".
    dataNoRealtime.ambient = 0;
    gi = UnityGI_BaseVRC(dataNoRealtime, s.Occlusion, s.Normal, -dataNoRealtime.worldViewDir, s.Smoothness, /*hasReflProbe*/ 0);

    // Minimal NaN/Inf guard (VRC_ApplyMinBrightness also did this).
    gi.indirect.diffuse = (gi.indirect.diffuse == gi.indirect.diffuse) ? gi.indirect.diffuse : 0;
    gi.indirect.diffuse = max(gi.indirect.diffuse, 0);

    // Remove all specular (probe hack + indirectSpecColor path).
    gi.indirect.specular = 0;

    // Remove all direct realtime lighting.
    gi.light.color = 0;
    gi.light.dir = 0;
}
...
{
    // Guard against NaNs/Infs coming from edge-case probe/lightmap values.
    // For NaN, (x == x) is false.
    gi.indirect.diffuse = (gi.indirect.diffuse == gi.indirect.diffuse) ? gi.indirect.diffuse : 0;
